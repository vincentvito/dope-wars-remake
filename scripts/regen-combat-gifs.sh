#!/usr/bin/env bash
# Regenerate combat GIFs from correct debug starting frames.
...
COMBAT_DIR="$BASE_DIR/public/sprites/combat"
DEBUG_DIR="$COMBAT_DIR/debug"

ANIMS=("combat-idle")

for name in "${ANIMS[@]}"; do
  src="$DEBUG_DIR/${name}-start.png"
  gif="$COMBAT_DIR/${name}.gif"
...
echo ""
echo "All combat GIFs regenerated!"
